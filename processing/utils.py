...
def perform_processing(image: np.ndarray) -> str:
    ...
    features_bank = [[0, 0], [0, 15], [15, 30], [30, 45], [45, 60], [60, 75], [75, 90], [90, 105]]
    gaussianWindowList = [1, 3, 5, 7, 9, 11, 13, 15]
    ...
    for bank_filter in range(len(features_bank)):
        for bank_filter_gauss in gaussianWindowList:

            imageCopy = increase_brightness_contrast(imageCopy, features_bank[bank_filter][0], features_bank[bank_filter][0])

            imgBlur = cv2.GaussianBlur(imageCopy, (bank_filter_gauss, bank_filter_gauss), gaussianDeviation)
                        ...
                        if area >= 10000 and area <= 120000:
                            ...
                            for i in range(len(contoursPlate)):
                                ...
                                if hPlate > dst.shape[0]/2 and wPlate < dst.shape[1]/4:
                                    ...
                                    # cv2.imwrite("image crop letter: " + str(i) + ".jpg", letter)

                                    letterIterator += 1

                            # cv2.imshow("image perspective transform", imageShow)
                            # cv2.imshow("image crop", dst)
                            # cv2.imwrite("image_crop.jpg", dst)
                            # cv2.waitKey(0)
